System.register(["d3"], function (_export, _context) {
  ...
  var d3__default, event, indicatormixin, atrtrailingstop, crosshair, ichimoku, macd, ohlc, rsi, trendline, value, volume, tick, trade, adx, aroon, stochastic, supstance, williams, bollinger, accessors, ema_init, sma_init, atr_init, CircularBuffer, util, sroc_init, vwap, atrtrailingstop$1, heikinashi, ichimoku$1, macd$1, rsi$1, adx$1, aroon$1, stochastic$1, williams$1, bollinger$1, indicators, arrow, svg, zoomable, financetime, scale, plot, plotmixin, candlestick, axisannotation, line, adx$2, aroon$2, atrtrailingstop$2, bollinger$2, crosshair$1, ichimoku$2, macd$2, ohlc$1, rsi$2, stochastic$2, supstance$1, tick$1, tradearrow, trendline$1, williams$2, volume$1, plot$1, techan;
  ...
  function datum$4(date, adx, plusDi, minusDi) {
    if (plusDi) {
      return {
        date: date,
        adx: adx,
  ...
  function datum$5(date, up, down, oscillator, middle, overbought, oversold) {
    if (up) return {
      date: date,
      up: up,
      down: down,
  ...
  function datum$6(date, stochasticK, stochasticD, overbought, oversold) {
    if (stochasticK) return {
      date: date,
  ...
  function datum$7(date, williams, middle, overbought, oversold) {
    if (williams) return {
      date: date,
      williams: williams,
      middle: middle,
  ...
  function mapReduceFilter(data, map) {
    return data.map(map).reduce(function (a, b) {
      return a.concat(b);
    }) // Flatten
    .filter(function (d) {
      return d !== null;
  ...
  function textAttributes(text, accessor, axis, orient, neg) {
    ...
    switch (orient) {
      case 'left':
      case 'right':
        text.attr('x', neg * (Math.max(axis.tickSizeInner(), 0) + axis.tickPadding())).attr('y', textPosition(accessor, scale)).attr('dy', '.32em').style('text-anchor', neg < 0 ? 'end' : 'start');
        break;

      case 'top':
      case 'bottom':
      default:
        text.attr('x', textPosition(accessor, scale)).attr('y', neg * (Math.max(axis.tickSizeInner(), 0) + axis.tickPadding())).attr('dy', neg < 0 ? '0em' : '.72em').style('text-anchor', 'middle');
        break;
    }
  }
  ...
  function refresh$1(selection, accessor, x, y, plot, svgLine, showZero) {
  ...
  function refresh$2(selection, adxLine, plusDiLine, minusDiLine) {
  ...
  function refresh$3(selection, accessor, x, y, plot, oscLine, oscArea, middleLine, upLine, downLine) {
  ...
  function refresh$4(selection, upLine, downLine) {
  ...
  function refresh$6(selection, accessor, x, y, plot, differenceGenerator, macdLine, signalLine) {
  ...
  function refresh$7(selection, accessor, x, y, plot, rsiLine) {
    selection.select('path.overbought').attr('d', plot.horizontalPathLine(accessor.d, x, accessor.ob, y));
  ...
  function refresh$8(selection, accessor, x, y, plot, kLine, dLine) {
    selection.select('path.overbought').attr('d', plot.horizontalPathLine(accessor.d, x, accessor.ob, y));
    selection.select('path.oversold').attr('d', plot.horizontalPathLine(accessor.d, x, accessor.os, y));
  ...
  function refresh$9(selection, accessor, x, y, annotationComposer) {
  ...
  function refresh$a(selection, accessor, x, y) {
  ...
  function trendlinePath(accessor, x, y) {
    return function (d) {
      return 'M ' + x(accessor.sd(d)) + ' ' + y(accessor.sv(d)) + ' L ' + x(accessor.ed(d)) + ' ' + y(accessor.ev(d));
    };
  }
  ...
  return {
    setters: [function (_d) {
      d3__default = _d.default;
      event = _d.event;
    }],
    execute: function () {
      indicatormixin = function () {
        return function (source, priv) {
          ...
          indicatorMixin.accessor = function (accessor) {
            priv.accessor = accessor; // Mixin the functions to the source
      ...
      stochastic = function () {
        var date = function (d) {
          return d.date;
            ...
            stochasticK = function (d) {
          return d.stochasticK;
        },
            stochasticD = function (d) {
          return d.stochasticD;
        },
            overbought = function (d) {
          return d.overbought;
        },
      ...
      ema_init = function (indicatorMixin, accessor_ohlc, alpha_init) {
        ...
        return function () {
          ...
          indicator.average = function (value) {
            if (initialCount < p.period) return previous = (initialTotal += value) / ++initialCount;else return previous = previous + alpha * (value - previous);
      ...
      sma_init = function (indicatorMixin, accessor_ohlc) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
      ...
      atr_init = function (indicatorMixin, accessor_ohlc, indicator_sma) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          function indicator(data) {
            ...
            return data.map(function (d, i) {
              ...
              if (i >= p.period) return datum(p.accessor.d(d), value);else return datum(p.accessor.d(d));
            }).filter(function (d) {
              return d.value !== null;
            });
          }
          ...
          indicator.atr = function (d) {
            var trueRange = previous === null ? p.accessor.h(d) - p.accessor.l(d) : Math.max(p.accessor.h(d) - p.accessor.l(d), Math.abs(p.accessor.h(d) - p.accessor.c(previous)), Math.abs(p.accessor.l(d) - p.accessor.c(previous)));
            previous = d; // http://en.wikipedia.org/wiki/Average_true_range
      ...
      CircularBuffer = function CircularBuffer(size) {
        ...
        CircularBuffer.push = function (value) {
          ...
          if (samples.length < size) samples.push(value);else samples[currentIndex] = value;
        };
      ...
      sroc_init = function (techan_util_circularbuffer, indicatorMixin, accessor_ohlc, indicator_smoothing, smoothed_period) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          function indicator(data) {
            indicator.init();
            return data.map(rocDataPoint).filter(function (d) {
              return d.value !== null;
            });
          }
      ...
      vwap = function (indicatorMixin, accessor_ohlc) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          function indicator(data) {
            indicator.init();
            return data.map(vwap).filter(function (d) {
              return d.value !== null;
            });
          }
          ...
          function vwap(d, i) {
            ...
            return {
              date: p.accessor.d(d),
              value: cumul_total / cumul_volume
            };
          } // Mixin 'superclass' methods and variables
      ...
      atrtrailingstop$1 = function (indicatorMixin, accessor_ohlc, indicator_atr) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          function indicator(data) {
            ...
            .reduce(function (result, d, i) {
              ...
              var prev = result[i - 1],
                  up = i === 0 ? d.up : null,
                  // Always start with an up trend?
              down = null;

              if (prev && prev.up !== null) {
                if (d.close > prev.up) up = Math.max(d.up, prev.up);else down = d.down;
              }

              if (prev && prev.down !== null) {
                if (d.close < prev.down) down = Math.min(d.down, prev.down);else up = d.up;
              }
      ...
      heikinashi = function (indicatorMixin, accessor_ohlc, min, max) {
        // Injected dependencies
        //Injected dependencies
        return function () {
          // Closure function  
      ...
      ichimoku$1 = function (indicatorMixin, accessor_ohlc) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          function indicator(data) {
            var parameters = {
              ...
              senkouSpanB: senkouSpanB
                ...
                result = new Array(data.length); // Iterate backwards through the data
          ...
          function calculate(parameters, data, index) {
            var d = data[index],
                min = p.accessor.l(d),
                max = p.accessor.h(d),
                current = datum$1(parameters, p.accessor.d(d), p.accessor.c(d)); // Iterate backwards through the data up to sendouSpanB count to calculate averages

            for (var i = 0, pos = i + 1; i < parameters.senkouSpanB && index - i >= 0; i++, pos = i + 1) {
              d = data[index - i];
              min = Math.min(min, p.accessor.l(d));
              max = Math.max(max, p.accessor.h(d)); // Grab a snapshot of average of min and max for each of the parameter periods
      ...
      macd$1 = function (indicatorMixin, accessor_ohlc, indicator_ema) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          function indicator(data) {
            ...
            return data.map(function (d, i) {
              ...
              if (i >= minCount) return datum$2(p.accessor.d(d), macd, signalValue, macd - signalValue, 0);else return datum$2(p.accessor.d(d));
            }).filter(function (d) {
              return d.macd !== null;
            });
          }
      ...
      rsi$1 = function (indicatorMixin, accessor_ohlc, indicator_ema) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
      ...
      adx$1 = function (d3_max, indicatorMixin, accessor_ohlc, indicator_ema) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {}; // Container for private, direct access mixed in variables
          ...
          function indicator(data) {
            ...
            return data.map(function (d, i) {
              if (i < 1) return datum$4(p.accessor.d(d));
              ...
              var TR = d3_max([p.accessor.h(d) - p.accessor.l(d), Math.abs(p.accessor.h(d) - p.accessor.c(data[i - 1])), Math.abs(p.accessor.l(d) - p.accessor.c(data[i - 1]))]);
              ...
              if (i > p.period) {
                ...
                if (i >= p.period * 2) {
                  return datum$4(p.accessor.d(d), adx, plusDi, minusDi);
      ...
      aroon$1 = function (indicatorMixin, accessor_ohlc) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          function indicator(data) {
            return data.map(function (d, i) {
              if (i >= p.period - 1) {
                ...
                return datum$5(p.accessor.d(d), up, down, oscillator, middle, overbought, oversold);
      ...
      stochastic$1 = function (indicatorMixin, accessor_ohlc) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          function indicator(data) {
            ...
            return data.map(function (d, i) {
              if (i >= periodLength) {
                ...
                stochasticD /= periodD;
                return datum$6(p.accessor.d(d), stochasticK, stochasticD, overbought, oversold);
              } else return datum$6(p.accessor.d(d), null, null, overbought, oversold);
            }).filter(function (d) {
              return d.stochasticK;
            });
          }
          ...
          indicatorMixin(indicator, p).accessor(accessor_ohlc()).period(20).preroll(function () {
      ...
      williams$1 = function (indicatorMixin, accessor_ohlc) {
        // Injected dependencies
        var p = {},
            // Container for private, direct access mixed in variables
        overbought = 80,
        ...
        function indicator(data) {
          return data.map(function (d, i) {
            if (i >= p.period) {
              ...
              return datum$7(p.accessor.d(d), williams, middle, overbought, oversold);
      ...
      bollinger$1 = function (indicatorMixin, accessor_ohlc, indicator_sma) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
      ...
      indicators = function (d3) {
        ...
        return {
          atr: atr,
          atrtrailingstop: atrtrailingstop$1(indicatorMixin, accessor.ohlc, atr),
          ema: ema,
          heikinashi: heikinashi(indicatorMixin, accessor.ohlc, d3.min, d3.max),
          ichimoku: ichimoku$1(indicatorMixin, accessor.ohlc),
          macd: macd$1(indicatorMixin, accessor.ohlc, ema),
          rsi: rsi$1(indicatorMixin, accessor.ohlc, ema),
          sma: sma,
          wilderma: ema_init(indicatorMixin, accessor.ohlc, wilder_alpha_init),
          aroon: aroon$1(indicatorMixin, accessor.ohlc),
          ...
          stochastic: stochastic$1(indicatorMixin, accessor.ohlc),
          williams: williams$1(indicatorMixin, accessor.ohlc),
          adx: adx$1(d3.max, indicatorMixin, accessor.ohlc, ema),
          bollinger: bollinger$1(indicatorMixin, accessor.ohlc, sma),
          vwap: vwap(indicatorMixin, accessor.ohlc)
        };
      };

      arrow = function (d3_functor) {
        // Injected dependencies
        return function () {
          var fx = d3_functor(0),
              fy = d3_functor(0),
          ...
          function arrow(d, i) {
            ...
            switch (o) {
              ...
              default:
                throw "Unsupported arrow.orient() = " + orient;
            }
      ...
      zoomable = function () {
        function zoomable(linear, zoomed, domainLimit, clamp) {
          ...
          scale.domain = function (_) {
            ...
            if (clamp) linear.domain([Math.max(domainLimit.domain[0], _[0]), Math.min(domainLimit.domain[1], _[1])]);else linear.domain(_);
      ...
      financetime = function (d3_scale_linear, d3_time, d3_bisect, techan_util_rebindCallback, scale_widen, techan_scale_zoomable) {
        // Injected dependencies
        function financetime(tickMethods, genericFormat, index, domain, padding, outerPadding, zoomLimit, closestTicks, zoomable) {
          ...
          zoomLimit = zoomLimit || {
            domain: index.domain()
          }; // Wrap in object to carry onto zoomable
          ...
          function scale(x, offset) {
            ...
            offset = offset || 0; // Make sure the value has been mapped, if not, determine if it's just before, round in, or just after domain
          ...
          scale.domain = function (_) {
            if (!arguments.length) {
              ...
              visible = [Math.max(Math.ceil(visible[0]), 0), // If min is fraction, it is partially out of view, but still partially visible, round up (ceil)
          ...
          function applyDomain() {
            ...
            zoomed(); // Apply outerPadding and widen the outer edges by pulling the domain in to ensure start and end bands are fully visible
          ...
          scale.ticks = function (interval, steps) {
            ...
            var method = interval === undefined ? tickMethod(visibleDomain, indexDomain, 10) : typeof interval === 'number' ? tickMethod(visibleDomain, indexDomain, interval) : null;
          ...
          function tickMethod(visibleDomain, indexDomain, count) {
            ...
            var visibleDomainExtent = visibleDomain[visibleDomain.length - 1] - visibleDomain[0],
                intraday = visibleDomainExtent / dailyStep < 1,
        ...
        var dailyStep = 864e5,
            dailyTickSteps = [dailyStep, // 1-day
        6048e5, // 1-week
        2592e6, // 1-month
        7776e6, // 3-month
        31536e6 // 1-year
        ],
            intradayTickSteps = [1e3, // 1-second
        5e3, // 5-second
        15e3, // 15-second
        3e4, // 30-second
        6e4, // 1-minute
        ...
        var dayFormat = d3_time.timeFormat('%b %e'),
            yearFormat = d3_v3_multi_shim([[d3_time.timeFormat('%b %Y'), function (d) {
          return d.getMonth();
        }], [d3_time.timeFormat('%Y'), function () {
          return true;
        }]]),
            intradayFormat = d3_v3_multi_shim([[d3_time.timeFormat(':%S'), function (d) {
          return d.getSeconds();
          ...
          return true;
            ...
            genericFormat = [d3_time.timeSecond, 1, d3_v3_multi_shim([[d3_time.timeFormat(':%S'), function (d) {
          return d.getSeconds();
        }], [d3_time.timeFormat('%I:%M'), function (d) {
          return d.getMinutes();
        ...
        var dayFormatUtc = d3_time.utcFormat('%b %e'),
            yearFormatUtc = d3_v3_multi_shim([[d3_time.utcFormat('%b %Y'), function (d) {
          return d.getUTCMonth();
        }], [d3_time.utcFormat('%Y'), function () {
          return true;
        }]]),
            intradayFormatUtc = d3_v3_multi_shim([[d3_time.utcFormat(':%S'), function (d) {
          return d.getUTCSeconds();
        }], [d3_time.utcFormat('%I:%M'), function (d) {
          return d.getUTCMinutes();
        }], [d3_time.utcFormat('%I %p'), function () {
          return true;
        }]]),
            genericFormatUtc = [d3_time.timeSecond, 1, d3_v3_multi_shim([[d3_time.utcFormat(':%S'), function (d) {
          return d.getUTCSeconds();
        }], [d3_time.utcFormat('%I:%M'), function (d) {
          return d.getUTCMinutes();
        }], [d3_time.utcFormat('%I %p'), function (d) {
          return d.getUTCHours();
        }], [d3_time.utcFormat('%b %e'), function () {
          return true;
        }]])];
        var dailyTickMethod = [[d3_time.timeDay, 1, dayFormat], [d3_time.timeMonday, 1, dayFormat], [d3_time.timeMonth, 1, yearFormat], [d3_time.timeMonth, 3, yearFormat], [d3_time.timeYear, 1, yearFormat]],
            intradayTickMethod = [[d3_time.timeSecond, 1, intradayFormat], [d3_time.timeSecond, 5, intradayFormat], [d3_time.timeSecond, 15, intradayFormat], [d3_time.timeSecond, 30, intradayFormat], [d3_time.timeMinute, 1, intradayFormat], [d3_time.timeMinute, 5, intradayFormat], [d3_time.timeMinute, 15, intradayFormat], [d3_time.timeMinute, 30, intradayFormat], [d3_time.timeHour, 1, intradayFormat], [d3_time.timeHour, 3, intradayFormat], [d3_time.timeHour, 6, intradayFormat], [d3_time.timeHour, 12, intradayFormat], [d3_time.timeDay, 1, dayFormat]];
        var dailyTickMethodUtc = [[d3_time.utcDay, 1, dayFormatUtc], [d3_time.utcMonday, 1, dayFormatUtc], [d3_time.utcMonth, 1, yearFormatUtc], [d3_time.utcMonth, 3, yearFormatUtc], [d3_time.utcYear, 1, yearFormatUtc]],
            intradayTickMethodUtc = [[d3_time.utcSecond, 1, intradayFormatUtc], [d3_time.utcSecond, 5, intradayFormatUtc], [d3_time.utcSecond, 15, intradayFormatUtc], [d3_time.utcSecond, 30, intradayFormatUtc], [d3_time.utcMinute, 1, intradayFormatUtc], [d3_time.utcMinute, 5, intradayFormatUtc], [d3_time.utcMinute, 15, intradayFormatUtc], [d3_time.utcMinute, 30, intradayFormatUtc], [d3_time.utcHour, 1, intradayFormatUtc], [d3_time.utcHour, 3, intradayFormatUtc], [d3_time.utcHour, 6, intradayFormatUtc], [d3_time.utcHour, 12, intradayFormatUtc], [d3_time.utcDay, 1, dayFormatUtc]];

        function techan_scale_financetime() {
          return financetime({
            daily: dailyTickMethod,
            intraday: intradayTickMethod
          }, genericFormat);
        }

        techan_scale_financetime.utc = function () {
          return financetime({
            daily: dailyTickMethodUtc,
            intraday: intradayTickMethodUtc
          }, genericFormatUtc);
        };
      ...
      scale = function (d3) {
        var zoomable$1 = zoomable(),
            util$1 = util(),
            accessors$1 = accessors(),
            financetime$1 = financetime(d3.scaleLinear, d3, d3.bisect, util$1.rebindCallback, widen, zoomable$1);

        function ohlc(data, accessor) {
          accessor = accessor || accessors$1.ohlc();
          return d3.scaleLinear().domain([d3.min(data.map(accessor.low())), d3.max(data.map(accessor.high()))].map(widen(0.02)));
        }

        function pathWithValueAccessor(data, accessor, widening) {
          accessor = accessor || accessors$1.value();
          widening = widening === undefined ? 0.02 : widening;
          return pathScale(d3, data, accessor, widening);
        }

        return {
          financetime: financetime$1,
          plot: {
            adx: function () {
            ...
            atrtrailingstop: function (data, accessor) {
              accessor = accessor || accessors$1.atrtrailingstop();
            ...
            bollinger: function (data, accessor) {
              accessor = accessor || accessors$1.bollinger();
              return d3.scaleLinear().domain([d3.min(data.map(function (d) {
                return accessor.lower(d);
              })), d3.max(data.map(function (d) {
                return accessor.upper(d);
              }))].map(widen(0.02)));
            },
            candlestick: ohlc,
            ...
            ichimoku: function (data, accessor) {
              accessor = accessor || accessors$1.ichimoku(); // Lots of values in each data point, assemble them together as they are plotted considering offsets, flatten, remove nulls

              var values = mapReduceFilter(data, function (d, i) {
                var chikouSpanData = data[i + accessor.pks(d)],
                    // Apply offset +pks (is plotted behind, so get data ahead)
                senkouSpanBData = data[i - accessor.pks(d)]; // Apply offset -pks (is plotted in front, so get data behind)

                return [accessor.ts(d), accessor.ks(d), senkouSpanBData ? accessor.sa(senkouSpanBData) : null, senkouSpanBData ? accessor.sb(senkouSpanBData) : null, chikouSpanData ? accessor.c(chikouSpanData) : null];
              });
              return d3.scaleLinear().domain(d3.extent(values).map(widen(0.02)));
            },
            macd: function (data, accessor) {
              accessor = accessor || accessors$1.macd();
            ...
            supstance: function (data, accessor) {
              accessor = accessor || accessors$1.supstance();
              return pathScale(d3, data, accessor.v, 0.02);
            },
            tick: ohlc,
            time: function (data, accessor) {
              accessor = accessor || accessors$1.value();
              return financetime$1().domain(data.map(accessor.d));
            },
            tradearrow: function (data, accessor) {
              accessor = accessor || accessors$1.trade();
              return pathScale(d3, data, accessor.p, 0.02);
            },
            trendline: function (data, accessor) {
              accessor = accessor || accessors$1.trendline();
              var values = mapReduceFilter(data, function (d) {
                return [accessor.sv(d), accessor.ev(d)];
              });
              return d3.scaleLinear().domain(d3.extent(values).map(widen(0.04)));
            },
            volume: function (data, accessor) {
              accessor = accessor || accessors$1.ohlc().v;
      ...
      plot = function (d3_svg_line, d3_svg_area, d3_line_interpolate, d3_select) {
        ...
        function PathArea() {
          ...
          area.init = function (accessor_date, x, accessor_value, y, yBase) {
            return d3Area.defined(function (d) {
              return accessor_value(d) !== null;
            }).x(function (d) {
              return x(accessor_date(d));
            }).y0(function (d) {
              return y(yBase);
            }).y1(function (d) {
              return y(accessor_value(d));
        ...
        function appendPlotTypePath(g, data, plotNames, direction) {
          g.selectAll('path.' + arrayJoin(plotNames, '.') + '.' + direction).data(function (d) {
            return [d.filter(data)];
          }).enter().append('path').attr('class', arrayJoin(plotNames, ' ') + ' ' + direction);
        }

        function barWidth(x) {
          if (x.band !== undefined) return Math.max(x.band(), 1);else return 3; // If it's not a finance time, the user should specify the band calculation (or constant) on the plot
        }
        ...
        function PlotComposer() {
          ...
          plotComposer.refresh = function (g) {
            dataSelector.select(g).data(function (d) {
              ...
              if (value === null || value === undefined) return plots.map(function () {
                return [];
              });
              var y = scale(value);
      ...
      plotmixin = function (d3_scale_linear, d3_functor, techan_scale_financetime, plot_dataselector, plot_width) {
      ...
      candlestick = function (d3_scale_linear, d3_extent, accessor_ohlc, plot, plotMixin) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          function candlestick(g) {
            var group = p.dataSelector(g); // 3x2 path's as wick and body can be styled slightly differently (stroke and fills)
          ...
          function bodyPath() {
            ...
            return function (d) {
              var open = y(accessor.o(d)),
                  ...
                  path = 'M ' + xValue + ' ' + open + ' l ' + width + ' ' + 0; // Draw body only if there is a body (there is no stroke, so will not appear anyway)
      ...
      axisannotation = function (d3_svg_axis, d3_scale_linear, accessor_value, plot, plotMixin) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
      ...
      line = function (accessor_value, plot, plotMixin, showZero) {
        // Injected dependencies
        ...
        return function () {
          ...
          line.refresh = function (g) {
            refresh$1(p.dataSelector.select(g), p.accessor, p.xScale, p.yScale, plot, svgLine, showZero);
          };
      ...
      adx$2 = function (accessor_adx, plot, plotMixin) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          adx.refresh = function (g) {
            refresh$2(p.dataSelector.select(g), adxLine, plusDiLine, minusDiLine);
          };
      ...
      aroon$2 = function (accessor_aroon, plot, plotMixin) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          aroon.refresh = function (g) {
            refresh$3(p.dataSelector.select(g), p.accessor, p.xScale, p.yScale, plot, oscLine, oscArea, middleLine, upLine, downLine);
          };
      ...
      atrtrailingstop$2 = function (accessor_atrtrailingstop, plot, plotMixin) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          atrtrailingstop.refresh = function (g) {
            refresh$4(p.dataSelector.select(g), upLine, downLine);
          };
      ...
      bollinger$2 = function (accessor_bollinger, plot, plotMixin) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
      ...
      crosshair$1 = function (d3_select, d3_event, d3_mouse, d3_dispatch, accessor_crosshair, plot, plotMixin) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          dispatcher = d3_dispatch('enter', 'out', 'move'),
              ...
              xAnnotationComposer = plot.plotComposer().scope('composed-annotation').plotScale(function (plot) {
            return plot.axis().scale();
          }),
              yAnnotationComposer = plot.plotComposer().scope('composed-annotation').plotScale(function (plot) {
            return plot.axis().scale();
          }),
              verticalWireRange,
              horizontalWireRange;
          ...
          crosshair.refresh = function (g) {
            ...
            g.selectAll('rect').attr('x', Math.min.apply(null, xRange)).attr('y', Math.min.apply(null, yRange)).attr('height', Math.abs(yRange[yRange.length - 1] - yRange[0])).attr('width', Math.abs(xRange[xRange.length - 1] - xRange[0])).on('mouseenter', function () {
              dispatcher.call('enter', this);
            }).on('mouseout', function () {
              dispatcher.call('out', this); // Redraw with null values to ensure when we enter again, there is nothing cached when redisplayed
          ...
          function mousemoveRefresh(selection, pathVerticalSelection, pathHorizontalSelection, xAnnotationSelection, yAnnotationSelection) {
            return function () {
          ...
          plotMixin(crosshair, p).plot(accessor_crosshair(), binder).dataSelector(function (d) {
            // Has the user set data? If not, put empty data ready for mouse over
            if (isEmpty(d)) return [initialiseWire()];else return [d];
          }).on(dispatcher);
      ...
      ichimoku$2 = function (d3_svg_area, d3_line_interpolate, accessor_ichimoku, plot, plotMixin) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          function kumoClipArea() {
            return d3_svg_area().curve(d3_line_interpolate).defined(function (d) {
      ...
      macd$2 = function (accessor_macd, plot, plotMixin) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          macd.refresh = function (g) {
            refresh$6(p.dataSelector.select(g), p.accessor, p.xScale, p.yScale, plot, differenceGenerator, macdLine, signalLine);
          };
      ...
      ohlc$1 = function (d3_scale_linear, d3_extent, accessor_ohlc, plot, plotMixin) {
        // Injected dependencies
        return function () {
          // Closure constructor
          var p = {},
      ...
      rsi$2 = function (accessor_rsi, plot, plotMixin) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          rsi.refresh = function (g) {
            refresh$7(p.dataSelector.select(g), p.accessor, p.xScale, p.yScale, plot, rsiLine);
          };
      ...
      stochastic$2 = function (accessor_stochastic, plot, plotMixin) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          stochastic.refresh = function (g) {
            refresh$8(p.dataSelector.select(g), p.accessor, p.xScale, p.yScale, plot, kLine, dLine);
          };
      ...
      supstance$1 = function (d3_behavior_drag, d3_event, d3_select, d3_dispatch, accessor_supstance, plot, plotMixin) {
        // Injected dependencies
        function Supstance() {
          // Closure function
          var p = {},
          ...
          dispatch = d3_dispatch('mouseenter', 'mouseout', 'mousemove', 'drag', 'dragstart', 'dragend'),
              annotationComposer = plot.plotComposer().scope('composed-annotation').plotScale(function (plot) {
            return plot.axis().scale();
          });
          ...
          function supstance(g) {
            ...
            var interaction = group.entry.append('g').attr('class', 'interaction').style('opacity', 0).style('fill', 'none').call(plot.interaction.mousedispatch(dispatch));
            interaction.append('path').style('stroke-width', '16px');
            supstance.refresh(g);
          }

          supstance.refresh = function (g) {
            refresh$9(p.dataSelector.select(g), p.accessor, p.xScale, p.yScale, annotationComposer);
          };
          ...
          plotMixin(supstance, p).dataSelector(plotMixin.dataMapper.unity).plot(accessor_supstance(), binder).on(dispatch); // Further group configuration now that it's mixed in
        ...
        function dragBody(dispatch, accessor, x, y, annotationComposer) {
          var drag = d3_behavior_drag().subject(function (d) {
            ...
            accessor.v(d, value);
            refresh$9(g, accessor, x, y, annotationComposer);
      ...
      tick$1 = function (d3_scale_linear, d3_extent, accessor_tick, plot, plotMixin) {
        // Injected dependencies
        return function () {
          // Closure constructor
          var p = {},
      ...
      tradearrow = function (d3_select, d3_functor, d3_mouse, d3_dispatch, accessor_trade, plot, plotMixin, svg_arrow) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          dispatch = d3_dispatch('mouseenter', 'mouseout'),
              y = function (d) {
            return p.yScale(p.accessor.p(d));
          },
              svgArrow = svg_arrow().orient(function (d) {
            return p.accessor.t(d) === 'buy' ? 'up' : 'down';
          }),
              arrowGenerator;
          ...
          function tradearrow(g) {
            ...
            group.selection.selectAll('path.tradearrow').on('mouseenter', function (data) {
              var nearest = findNearest(data, d3_mouse(this)[0]); // Watch out here, not using generator as this is single element, not grouped
          ...
          function binder() {
            svgArrow.x(function (d) {
              return p.xScale(p.accessor.d(d));
            }).y(y);
            arrowGenerator = plot.joinPath(function () {
              return svgArrow;
            });
            return tradearrow;
          }
          ...
          function typesToClasses(data) {
            return data.map(function (d) {
              return p.accessor.t(d);
            }).reduce(function (prev, cur) {
              if (prev[cur] === undefined) prev[cur] = function (d) {
                return cur === p.accessor.t(d);
              };
              return prev;
            }, {});
          } // Mixin 'superclass' methods and variables
      ...
      trendline$1 = function (d3_behavior_drag, d3_event, d3_select, d3_dispatch, accessor_trendline, plot, plotMixin) {
        // Injected dependencies
        function Trendline() {
          // Closure function
          var p = {},
          ...
          trendline.refresh = function (g) {
            refresh$a(p.dataSelector.select(g), p.accessor, p.xScale, p.yScale);
          };

          trendline.drag = function (g) {
            g.selectAll('.interaction circle.start').call(dragEnd(dispatch, p.accessor, p.accessor.sd, p.xScale, p.accessor.sv, p.yScale));
            g.selectAll('.interaction circle.end').call(dragEnd(dispatch, p.accessor, p.accessor.ed, p.xScale, p.accessor.ev, p.yScale));
            g.selectAll('.interaction path.body').call(dragBody(dispatch, p.accessor, p.xScale, p.yScale));
          }; // Mixin 'superclass' methods and variables


          plotMixin(trendline, p).dataSelector(plotMixin.dataMapper.unity).plot(accessor_trendline()).on(dispatch);
          return trendline;
        }
        ...
        function dragEnd(dispatch, accessor, accessor_x, x, accessor_y, y) {
          ...
          drag.subject(function (d) {
            ...
            refresh$a(d3_select(this.parentNode.parentNode.parentNode), accessor, x, y);
            dispatch.call('drag', this, d);
          });
          return plot.interaction.dragStartEndDispatch(drag, dispatch);
        }
        ...
        function dragBody(dispatch, accessor, x, y) {
          ...
          drag.subject(function (d) {
            dragStart.start = {
              date: x(accessor.sd(d)),
              value: y(accessor.sv(d))
            };
            dragStart.end = {
              date: x(accessor.ed(d)),
              value: y(accessor.ev(d))
            };
            return {
              x: 0,
              y: 0
            ...
            updateEnd(accessor.sd, x, d3_event().x + dragStart.start.date, accessor.sv, y, d3_event().y + dragStart.start.value, d);
            updateEnd(accessor.ed, x, d3_event().x + dragStart.end.date, accessor.ev, y, d3_event().y + dragStart.end.value, d);
            refresh$a(d3_select(this.parentNode.parentNode.parentNode), accessor, x, y);
            dispatch.call('drag', this, d);
          });
          return plot.interaction.dragStartEndDispatch(drag, dispatch);
      ...
      williams$2 = function (accessor_williams, plot, plotMixin) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
      ...
      volume$1 = function (accessor_volume, plot, plotMixin) {
        // Injected dependencies
        return function () {
          // Closure function
          var p = {},
          ...
          function volume(g) {
            var group = p.dataSelector(g);
            if (isOhlcAccessor()) plot.appendPathsUpDownEqual(group.selection, p.accessor, 'volume');else group.entry.append('path').attr('class', 'volume');
            volume.refresh(g);
          }

          volume.refresh = function (g) {
            if (isOhlcAccessor()) g.selectAll('path.volume').attr('d', volumeGenerator);else p.dataSelector.select(g).select('path.volume').attr('d', volumeGenerator);
          };
      ...
      plot$1 = function (d3) {
        var scale$1 = scale(d3),
            accessor = accessors(),
            plot$1 = plot(d3.line, d3.area, d3.curveMonotoneX, d3.select),
            d3_functor = util().functor,
            plotMixin = plotmixin(d3.scaleLinear, d3_functor, scale$1.financetime, plot$1.dataSelector, plot$1.barWidth),
            candlestick$1 = candlestick(d3.scaleLinear, d3.extent, accessor.ohlc, plot$1, plotMixin),
            axisannotation$1 = axisannotation(d3.axisTop, d3.scaleLinear, accessor.value, plot$1, plotMixin),
            svg$1 = svg();
        return {
          adx: adx$2(accessor.adx, plot$1, plotMixin),
          aroon: aroon$2(accessor.aroon, plot$1, plotMixin),
          atr: line(accessor.value, plot$1, plotMixin),
          atrtrailingstop: atrtrailingstop$2(accessor.atrtrailingstop, plot$1, plotMixin),
          axisannotation: axisannotation$1,
          bollinger: bollinger$2(accessor.bollinger, plot$1, plotMixin),
          candlestick: candlestick$1,
          close: line(accessor.ohlc, plot$1, plotMixin),
          crosshair: crosshair$1(d3.select, d3_event, d3.mouse, d3.dispatch, accessor.crosshair, plot$1, plotMixin),
          ema: line(accessor.value, plot$1, plotMixin),
          heikinashi: candlestick$1,
          ichimoku: ichimoku$2(d3.area, d3.curveMonotoneX, accessor.ichimoku, plot$1, plotMixin),
          macd: macd$2(accessor.macd, plot$1, plotMixin),
          momentum: line(accessor.value, plot$1, plotMixin, true),
          moneyflow: line(accessor.value, plot$1, plotMixin, true),
          ohlc: ohlc$1(d3.scaleLinear, d3.extent, accessor.ohlc, plot$1, plotMixin),
          roc: line(accessor.value, plot$1, plotMixin, true),
          rsi: rsi$2(accessor.rsi, plot$1, plotMixin),
          sma: line(accessor.value, plot$1, plotMixin),
          sroc: line(accessor.value, plot$1, plotMixin, true),
          stochastic: stochastic$2(accessor.stochastic, plot$1, plotMixin),
          supstance: supstance$1(d3.drag, d3_event, d3.select, d3.dispatch, accessor.supstance, plot$1, plotMixin),
          tick: tick$1(d3.scaleLinear, d3.extent, accessor.tick, plot$1, plotMixin),
          tradearrow: tradearrow(d3.select, d3_functor, d3.mouse, d3.dispatch, accessor.trade, plot$1, plotMixin, svg$1.arrow),
          trendline: trendline$1(d3.drag, d3_event, d3.select, d3.dispatch, accessor.trendline, plot$1, plotMixin),
          volume: volume$1(accessor.volume, plot$1, plotMixin),
          vwap: line(accessor.value, plot$1, plotMixin),
          wilderma: line(accessor.value, plot$1, plotMixin),
          williams: williams$2(accessor.williams, plot$1, plotMixin)
        };
      };
      ...
      techan = {
        ...
        accessor: accessors(),
        indicator: indicators(d3__default),
        plot: plot$1(d3__default),
        scale: scale(d3__default),
        svg: svg()
      };

      _export("default", techan);
    }
  };
});
